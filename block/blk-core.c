...
blk_qc_t submit_bio(struct bio *bio)
{
	...
	if (bio_has_data(bio)) {
		...
		if (unlikely(block_dump)) {
			char b[BDEVNAME_SIZE];
			static pid_t _prev_pid = 0;
			if (task_pid_nr(current) != _prev_pid && strncmp(bdevname(bio->bi_bdev, b), "md", 2)==0 && strncmp(current->comm, "jbd2", 4)!=0 && strncmp(current->comm, "kworker", 7)!=0 ) {
